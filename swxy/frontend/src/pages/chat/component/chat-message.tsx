...
import { EvidenceComplete, EvidenceSearching } from './select-file'
      ...
      {list.map((item, index) => {
        ...
        return (
          <AssistantMessage
            ...
            onRefrence={(citationId) => {
              const target = item.reference?.find(
                (reference) => reference.citation_id === citationId,
              )
